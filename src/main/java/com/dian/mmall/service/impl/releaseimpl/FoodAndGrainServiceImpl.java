...
public class FoodAndGrainServiceImpl implements FoodAndGrainService {
	...
	public ServerResponse<Object> get_myFoodAndGrain_list(User user, Map<String, Object> params) {
	    ...
	    List<FoodAndGrain> list_equipment  =	new ArrayList();
	    List<FoodAndGrain> list_equipmentall=  equipmentMapper.get_myFoodAndGrain_list((currentPage-1)*pageSize,pageSize,releaseType,welfareStatus,user.getId());
	    
		for(FoodAndGrain equipment :list_equipmentall) {
	...
	public ServerResponse<String> operation_userFoodAndGrain(User user, Map<String, Object> params) {
		...
		if(type!=null && !type.equals("") && userId!=null && !userId.equals("")
			...
			if(type_int==1 || type_int==2) {
				...
				result=equipmentMapper.operation_userFoodAndGrain(userIdLong,idLong,type_int,timeString,termOfValidity);
				...
				//检查图片
				ServerResponse<String> serverResponseString=setPictureUrl((ArrayList<Picture>) params.get("pictureUrl"), foodAndGrain.getPictureUrl());
	...
	public ServerResponse<Object> getFoodAndGrainTitleList(Map<String, Object> params) {
		...
		String releaseTypeString = params.get("releaseType").toString().trim();
	 	...
	 	if(typeString!=null && !typeString.equals("")) {
	      ...
	      List<String> equipmentReleaseTitleList=equipmentMapper.getFoodAndGrainTitleList(releaseType,detailed,serviceType,releaseTitle,type);
		if(equipmentReleaseTitleList.size()==0) {
			detailed="%"+cityMapper.checkeCityTuo(provincesId, cityId)+"%";
			equipmentReleaseTitleList=equipmentMapper.getFoodAndGrainTitleList(releaseType,detailed,serviceType,releaseTitle,type);
		}
	...
	public ServerResponse<Object> getFoodAndGrainPublicList(Map<String, Object> params) {
		...
		String serviceType= params.get("serviceType").toString().trim();
	 	...
	 	long zongtiaoshu=equipmentMapper.getFoodAndGrainPublicListNo(releaseType,detailed,releaseTitle,serviceType);
		
	 	if(zongtiaoshu==0) {
	 		detailed="%"+cityMapper.checkeCityTuo(provincesId, cityId)+"%";
	 		zongtiaoshu=equipmentMapper.getFoodAndGrainPublicListNo(releaseType,detailed,releaseTitle,serviceType);
	 	}
		...
		equipment_pagePage.setCurrentPage(currentPage); //当前页
	   //查询list
	 List<FoodAndGrain>	equipmentList=equipmentMapper.getFoodAndGrainPublicList((currentPage-1)*pageSize,releaseType,pageSize,releaseTitle,detailed,serviceType);
	...
	public ServerResponse<Object> adminFoodAndGrain(Map<String, Object> params) {
	    ...
	    List<FoodAndGrain> list_equipment  =	new ArrayList();
	    List<FoodAndGrain> list_equipmentall=  equipmentMapper.adminFoodAndGrain((currentPage-1)*pageSize,pageSize,contact,releaseType);
	    
	    List<ServiceType> serviceTypeList=serviceTypeMapper.get_serviceTypeAll(releaseType);
	    
		for(FoodAndGrain re :list_equipmentall) {
