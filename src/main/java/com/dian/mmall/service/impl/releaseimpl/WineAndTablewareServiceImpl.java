...
public class WineAndTablewareServiceImpl implements WineAndTablewareService {
	...
	public ServerResponse<String> create_wineAndTableware(User user, Map<String, Object> params) {
		ServerResponse<Object> serverResponse=check_evaluate(user, params,1);
	       ...
	       WineAndTableware wineAndTableware=(WineAndTableware) BeanMapConvertUtil.convertMap(WineAndTableware.class,map);
...
public ServerResponse<Object> get_myWineAndTableware_list(User user, Map<String, Object> params) {
	...
	int  pageSize=0;
 	...
 	long zongtiaoshu=wineAndTablewareMapper.get_myWineAndTableware_list_no(releaseType,welfareStatus,user.getId());
    ...
    List<WineAndTableware> list_equipment  =	new ArrayList();
    List<WineAndTableware> list_equipmentall=  wineAndTablewareMapper.get_myWineAndTableware_list((currentPage-1)*pageSize,pageSize,releaseType,welfareStatus,user.getId());
    
	for(WineAndTableware equipment :list_equipmentall) {
...
public ServerResponse<String> operation_userWineAndTableware(User user, Map<String, Object> params) {
	...
	if(type!=null && !type.equals("") && userId!=null && !userId.equals("")
		...
		if(type_int==1) {
			...
			result=wineAndTablewareMapper.operation_userWineAndTableware(userIdLong,idLong,type_int,timeString);
			...
			serverResponse=check_evaluate(user, params,2);
		       ...
		       WineAndTableware equipment_create=(WineAndTableware) BeanMapConvertUtil.convertMap(WineAndTableware.class,map);
			 	//{result=true, message=验证通过} 返回结果
...
public ServerResponse<Object> adminWineAndTableware(Map<String, Object> params) {
    ...
    List<WineAndTableware> list_equipment  =	new ArrayList();
    List<WineAndTableware> list_equipmentall=  wineAndTablewareMapper.adminWineAndTableware((currentPage-1)*pageSize,pageSize,contact,releaseType);
    
    List<ServiceType> serviceTypeList=serviceTypeMapper.get_serviceTypeAll(releaseType);
    
	for(WineAndTableware re :list_equipmentall) {
