...
public class EquipmentServiceImpl implements EquipmentService {
	...
	public ServerResponse<Object> adminEquipment(Map<String, Object> params) {
		...
		List<Equipment> list_equipmentall = equipmentMapper.adminEquipment((currentPage - 1) * pageSize, pageSize,
				contact);
	...
	public ServerResponse<Object> get_myEquipment_list(User user, Map<String, Object> params) {
		...
		List<Equipment> list_equipmentall = equipmentMapper.get_myEquipment_list((currentPage - 1) * pageSize, pageSize,
				releaseType, welfareStatus, user.getId());
		RealName realName = realNameMapper.getRealName(user.getId());

		for (int a = 0; a < list_equipmentall.size(); a++) {
			Equipment equipment = list_equipmentall.get(a);
	...
	public ServerResponse<Object> getEquipmentReleaseTitleList(Map<String, Object> params) {
		...
		if (selectedOptions_list.size() == 3) {
			...
			List<String> equipmentReleaseTitleList = equipmentMapper.getEquipmentReleaseTitleList(releaseType, detailed,
					serviceType, releaseTitle, type);
			if (equipmentReleaseTitleList.size() == 0) {
				detailed = "%" + cityMapper.checkeCityTuo(provincesId, cityId) + "%";
				equipmentReleaseTitleList = equipmentMapper.getEquipmentReleaseTitleList(releaseType, detailed,
						serviceType, releaseTitle, type);
			}
	...
	public ServerResponse<Object> getEquipmentPublicList(Map<String, Object> params) {
		...
		if (selectedOptions_list.size() == 3) {
			...
			equipment_pagePage.setCurrentPage(currentPage); // 当前页
			// 查询list
			List<Equipment> equipmentList = equipmentMapper.getEquipmentPublicList((currentPage - 1) * pageSize,
