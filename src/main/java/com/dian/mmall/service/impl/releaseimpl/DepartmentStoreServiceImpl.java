...
public class DepartmentStoreServiceImpl implements DepartmentStoreService {
	...
	public ServerResponse<Object> get_myDepartmentStore_list(User user, Map<String, Object> params) {
		...
		int  pageSize=0;
	 	...
	 	long zongtiaoshu=departmentStoreMapper.get_myDepartmentStore_list_no(releaseType,welfareStatus,user.getId());
	...
	public ServerResponse<String> operation_userDepartmentStore(User user, Map<String, Object> params) {
		...
		if(type!=null && !type.equals("") && userId!=null && !userId.equals("")
			...
			if(type_int==1 || type_int==2) {
				...
				result=departmentStoreMapper.operation_userDepartmentStore(userIdLong,idLong,type_int,timeString,termOfValidity);
				...
				serverResponse=check_evaluate(user, params,2);
			       ...
			       DepartmentStore equipment_create=(DepartmentStore) BeanMapConvertUtil.convertMap(DepartmentStore.class,map);
				 	//{result=true, message=验证通过} 返回结果
	...
	public ServerResponse<Object> getDepartmentStoreTitleList(Map<String, Object> params) {
		...
		String releaseTypeString = params.get("releaseType").toString().trim();
	 	...
	 	if(typeString!=null && !typeString.equals("")) {
	      ...
	      List<String> equipmentReleaseTitleList=departmentStoreMapper.getDepartmentStoreTitleList(releaseType,detailed,serviceType,releaseTitle,type);
		if(equipmentReleaseTitleList.size()==0) {
			detailed="%"+cityMapper.checkeCityTuo(provincesId, cityId)+"%";
			equipmentReleaseTitleList=departmentStoreMapper.getDepartmentStoreTitleList(releaseType,detailed,serviceType,releaseTitle,type);
		}
	...
	public ServerResponse<Object> getDepartmentStorePublicList(Map<String, Object> params) {
		...
		String serviceType= params.get("serviceType").toString().trim();
	 	...
	 	long zongtiaoshu=departmentStoreMapper.getDepartmentStorePublicListNo(releaseType,detailed,releaseTitle,serviceType);
		
	 	if(zongtiaoshu==0) {
	 		detailed="%"+cityMapper.checkeCityTuo(provincesId, cityId)+"%";
	 		zongtiaoshu=departmentStoreMapper.getDepartmentStorePublicListNo(releaseType,detailed,releaseTitle,serviceType);
	 	}
		...
		equipment_pagePage.setCurrentPage(currentPage); //当前页
	   //查询list
	 List<DepartmentStore>	equipmentList=departmentStoreMapper.getDepartmentStorePublicList((currentPage-1)*pageSize,releaseType,pageSize,releaseTitle,detailed,serviceType);
