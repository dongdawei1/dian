...
public class RentServiceImpl implements RentService {
	...
	public ServerResponse<Object> get_myRent_list(User user, Map<String, Object> params) {
	    ...
	    List<Rent> list_rent  =	new ArrayList();
	    List<Rent> list_rentall=  rentMapper.get_myRent_list((currentPage-1)*pageSize,pageSize,welfareStatus,user.getId());
	    
		for(Rent rent_no :list_rentall) {
			rent_no.setContact(EncrypDES.decryptPhone(rent_no.getContact()));
			list_rent.add(rent_no);
		}

		rent.setDatas(list_rent );
	...
	public ServerResponse<Object> adminMent(Map<String, Object> params) {
		...
		for(Rent re :list_rentall) {
			re.setContact(EncrypDES.decryptPhone(re.getContact()));
			list_rent.add(re);
		}

		rent_pagePage.setDatas(list_rent );
	...
	public ServerResponse<String> operation_userment(User user, Map<String, Object> params) {
		...
		if(type!=null && !type.equals("") && userId!=null && !userId.equals("")
			...
			if(type_int==1 || type_int==2) {
				...
				//检查图片
				ServerResponse<String> serverResponseString=setPictureUrl((ArrayList<Picture>) params.get("pictureUrl"), rent.getPictureUrl());
